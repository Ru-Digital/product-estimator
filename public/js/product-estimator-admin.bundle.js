  ...
  return (0,_babel_runtime_helpers_createClass__WEBPACK_IMPORTED_MODULE_1__["default"])(HierarchicalLabelSettingsModule, [{
    ...
    value: function expandAllSections($panel) {
      ...
      $headings.each(function (_, heading) {
        ...
        if (path) {
          _this4.expandedSections.add(path);
          _this4.updateSectionVisibility(path, true);
        }
      });
    }
    ...
    value: function collapseAllSections($panel) {
      ...
      $headings.each(function (_, heading) {
        ...
        if (path) {
          ...
          _this5.updateSectionVisibility(path, false);
        }
      });
    }
    ...
    value: function handleSearch(e) {
      ...
      this.searchResults.forEach(function (result) {
        var highlightedPath = _this6.highlightSearchTerm(result.path, searchTerm);
        var highlightedValue = _this6.highlightSearchTerm(result.value, searchTerm);
    ...
    value: function goToLabelField(path) {
      ...
      pathParts.forEach(function (part, index) {
        ...
        _this7.expandedSections.add(currentPath);
        _this7.updateSectionVisibility(currentPath, true);
      });
    ...
    value: function initializeUI() {
      ...
      jQuery('.pe-label-field-wrapper[data-depth]').each(function (_, field) {
        ...
        if (pathParts.length > 2) {
          ...
          var isVisible = _this8.expandedSections.has(parentPath);
      ...
      jQuery('.pe-label-subcategory-heading').each(function (_, heading) {
        ...
        var $data = $heading.next('.pe-label-subcategory-data');
        var path = $data.data('path');
        if (path && _this8.expandedSections.has(path)) {
          $heading.addClass('expanded');
        } else {
          $heading.removeClass('expanded');
        }
...
jQuery(document).ready(function () {
  if (jQuery('#labels').length) {
    ...
    var labelSettings = ((_window$productEstima = window.productEstimatorSettings) === null || _window$productEstima === void 0 ? void 0 : _window$productEstima.labelSettings) || {};
...
var LabelSettingsModule = /*#__PURE__*/function (_VerticalTabbedModule) {
  ...
  return (0,_babel_runtime_helpers_createClass__WEBPACK_IMPORTED_MODULE_2__["default"])(LabelSettingsModule, [{
    ...
    value: function bindModuleSpecificEvents() {
      ...
      // Preview updates
      this.$('.regular-text[id^="buttons_"], .regular-text[id^="forms_"], .regular-text[id^="messages_"], .regular-text[id^="ui_elements_"], .regular-text[id^="pdf_"]').on('input', this.updatePreview.bind(this));
    ...
    value: function handleResetCategory() {
      ...
      var validCategories = ['buttons', 'forms', 'messages', 'ui_elements', 'pdf'];
    ...
    value: function initializePreview() {
      ...
      jQuery('.regular-text[id^="buttons_"], .regular-text[id^="forms_"], .regular-text[id^="messages_"], .regular-text[id^="ui_elements_"], .regular-text[id^="pdf_"]').each(function (index, element) {
