...
var VerticalTabbedModule = /*#__PURE__*/function (_ProductEstimatorSett) {
  ...
  return (0,_babel_runtime_helpers_createClass__WEBPACK_IMPORTED_MODULE_2__["default"])(VerticalTabbedModule, [{
    ...
    key: "getCurrentSubTabId",
    value: function getCurrentSubTabId() {
      if (!this.$container || !this.$container.length) {
      ...
      var $verticalTabsNav = this.$container.find(navSelector);
      var $activeNavItem = $verticalTabsNav.find("".concat(navItemSelector, ".active"));
      logger.log("DEBUG getCurrentSubTabId: Found nav: ".concat($verticalTabsNav.length, ", active items: ").concat($activeNavItem.length));
      if ($activeNavItem.length) {
        var $activeLink = $activeNavItem.find('a');
        logger.log("DEBUG getCurrentSubTabId: Found active link: ".concat($activeLink.length));
        if ($activeLink.length) {
          // Try to get the tab ID from data-tab attribute
          var tabId = $activeLink.attr('data-tab');
          var href = $activeLink.attr('href') || '';
          var linkText = $activeLink.text().trim();
          logger.log("DEBUG getCurrentSubTabId: Link text: \"".concat(linkText, "\", data-tab: \"").concat(tabId, "\", href: \"").concat(href, "\""));
          if (!tabId) {
            // Fall back to extracting from href
            var subTabMatch = href.match(/[?&]sub_tab=([^&#]*)/i);
            if (subTabMatch && subTabMatch[1]) {
              tabId = decodeURIComponent(subTabMatch[1].replace(/\+/g, ' '));
              logger.log("DEBUG getCurrentSubTabId: Extracted from href: \"".concat(tabId, "\""));
            }
          }
          logger.log("DEBUG getCurrentSubTabId: Final tabId: \"".concat(tabId, "\""));
          return tabId || null;
        }
      } else {
        // Debug: show all nav items and their states
        var $allNavItems = $verticalTabsNav.find(navItemSelector);
        logger.log("DEBUG getCurrentSubTabId: All nav items (".concat($allNavItems.length, "):"));
        $allNavItems.each(function (i, item) {
          var $item = jQuery(item);
          var $link = $item.find('a');
          var classes = $item.attr('class') || '';
          var linkText = $link.text().trim();
          var isActive = $item.hasClass('active');
          logger.log("  - \"".concat(linkText, "\": classes=\"").concat(classes, "\", active=").concat(isActive));
        });
      }
      return null;
    }
    ...
    key: "activateSubTab",
    value: function activateSubTab(subTabId) {
      var _this$$container;
      logger.log("DEBUG: activateSubTab called with: ".concat(subTabId));
      logger.log("DEBUG: Container exists: ".concat(!!((_this$$container = this.$container) !== null && _this$$container !== void 0 && _this$$container.length)));
      logger.log("DEBUG: Settings tab_id: ".concat(this.settings.tab_id));
    ...
    value: function showVerticalTab(subTabId) {
      ...
      logger.log("DEBUG: showVerticalTab called with: ".concat(subTabId, ", updateHistory: ").concat(updateHistory));
      if (!this.$container || !this.settings.tab_id || !subTabId) {
        ...
        logger.warn('VerticalTabbedModule: Cannot show vertical tab due to missing container, mainTabId, or subTabId.', {
          containerExists: !!((_this$$container2 = this.$container) !== null && _this$$container2 !== void 0 && _this$$container2.length),
      ...
      var $verticalTabsNav = this.$container.find(navSelector);
      var $verticalTabContents = this.$container.find(contentSelector);
      logger.log("DEBUG: Found nav elements: ".concat($verticalTabsNav.length));
      logger.log("DEBUG: Found content elements: ".concat($verticalTabContents.length));
      logger.log("DEBUG: Using nav selector: ".concat(navSelector));
      logger.log("DEBUG: Using nav item selector: ".concat(navItemSelector));
      ...
      logger.log("DEBUG: Found link with data-tab=\"".concat(subTabId, "\": ").concat($activeLink.length));

      // Debug: show all available tabs
      var $allTabLinks = $verticalTabsNav.find('a');
      logger.log("DEBUG: Available tabs:");
      $allTabLinks.each(function (i, link) {
        var $link = jQuery(link);
        var dataTab = $link.attr('data-tab');
        var href = $link.attr('href') || '';
        var text = $link.text().trim();
        logger.log("  - \"".concat(text, "\": data-tab=\"").concat(dataTab, "\", href=\"").concat(href, "\""));
      });
      ...
      if ($activeLink.length) {
        var $navItem = $activeLink.closest(navItemSelector);
        logger.log("DEBUG: Found nav item to activate: ".concat($navItem.length, ", classes before: ").concat($navItem.attr('class')));
        $navItem.addClass('active');
        logger.log("DEBUG: Nav item classes after activation: ".concat($navItem.attr('class')));
      } else {
        // No active link found - this is OK if this is the initial load
        // and we'll select the default tab
        logger.log("DEBUG: No active link found for tab: ".concat(subTabId));
      }
      ...
      if ($activeContentPanel.length) {
        logger.log("DEBUG: Activating content panel with ID: ".concat($activeContentPanel.attr('id'), " and classes: ").concat($activeContentPanel.attr('class')));
        $activeContentPanel.show().addClass('active');
        logger.log("DEBUG: Content panel after activation - display: ".concat($activeContentPanel.css('display'), ", classes: ").concat($activeContentPanel.attr('class')));
      } else {
...
/* harmony import */ var _modules_SimilarProductsSettingsModule__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ./modules/SimilarProductsSettingsModule */ "./src/js/admin/modules/SimilarProductsSettingsModule.js");
/**
...
var logger = (0,_utils__WEBPACK_IMPORTED_MODULE_7__.createLogger)('LabelSettingsModule');
var LabelSettingsModule = /*#__PURE__*/function (_VerticalTabbedModule) {
  ...
  return (0,_babel_runtime_helpers_createClass__WEBPACK_IMPORTED_MODULE_2__["default"])(LabelSettingsModule, [{
    ...
    value: function initializeHierarchicalSections() {
      ...
      var $subcategoryHeadings = jQuery('.pe-label-subcategory-heading');
      var $mainSectionHeaders = jQuery('.pe-main-section-header');
      var $allHeadings = $subcategoryHeadings.add($mainSectionHeaders);
      ...
      $allHeadings.each(function (_, heading) {
      ...
      var $fieldWrappers = jQuery('.pe-label-field-wrapper');
      logger.log('Found field wrappers:', $fieldWrappers.length);
      $fieldWrappers.each(function (_, wrapper) {
        var $wrapper = jQuery(wrapper);
        var fieldPath = $wrapper.find('input[data-path]').data('path');
        if (fieldPath) {
          ...
          if (pathParts.length > 1) {
            ...
            for (var i = pathParts.length - 1; i > 0; i--) {
              ...
              var $parentSection = jQuery(".pe-label-subcategory-data[data-path=\"".concat(parentPath, "\"]"));
    ...
    value: function addExpandCollapseButtons() {
      ...
      this.$('.pe-vtabs-tab-panel, .vertical-tab-content').each(function (_, panel) {
        ...
        var $mainSectionHeaders = $panel.find('.pe-main-section-header');
        var $allCollapsibleHeadings = $subcategoryHeadings.add($mainSectionHeaders);
    ...
    value: function toggleSubcategory(e) {
      ...
      var path = $heading.attr('data-path') || $heading.next('.pe-label-subcategory-data').data('path');
    ...
    value: function expandNestedSections(mainSectionPath) {
      ...
      var $nestedHeadings = jQuery('.pe-label-subcategory-heading').filter(function () {
        var $heading = jQuery(this);
        var headingPath = $heading.next('.pe-label-subcategory-data').data('path') || $heading.data('path');
        return headingPath && headingPath.startsWith(mainSectionPath + '.');
      });
      $nestedHeadings.each(function (_, heading) {
        var $heading = jQuery(heading);
    ...
    value: function updateSectionVisibility(path, visible) {
      var skipNestedToggle = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
      logger.log("Updating visibility for path: ".concat(path, ", visible: ").concat(visible));

      // Find the heading - could be either subcategory or main section header
      var $heading = jQuery(".pe-label-subcategory-heading[data-path=\"".concat(path, "\"]"));
      if (!$heading.length) {
        $heading = jQuery(".pe-main-section-header[data-path=\"".concat(path, "\"]"));
      }
      var $indicator = $heading.find('.expand-indicator');
      ...
      var $fields = jQuery(".pe-label-field-wrapper[data-parent-section=\"".concat(path, "\"]"));
      $allFields = $allFields.add($fields);

      // Strategy 2: Direct child fields using path prefix matching
      var $directFields = jQuery(".pe-label-field-wrapper").filter(function () {
        var fieldPath = jQuery(this).attr('data-field-path') || '';
      ...
      var $inputFields = jQuery("input[data-path^=\"".concat(path, ".\"]")).closest('.pe-label-field-wrapper');
      ...
      if ($heading.hasClass('pe-label-subcategory-heading')) {
        var $nextSubcategoryOrMain = $heading.nextAll('.pe-label-subcategory-heading, .pe-main-section-header').first();
        if ($nextSubcategoryOrMain.length > 0) {
          // Include all content between this subcategory and the next heading
          var $contentBetween = $heading.nextUntil($nextSubcategoryOrMain);
          $allFields = $allFields.add($contentBetween);
        } else {
          // If no next heading, take all following content until a main section
          var $nextMainHeader = $heading.nextAll('.pe-main-section-header').first();
          if ($nextMainHeader.length > 0) {
            var $contentUntilMain = $heading.nextUntil($nextMainHeader);
      ...
      var $fieldWrappers = $allFields.filter('.pe-label-field-wrapper');
      $fieldWrappers.each(function () {
        var $wrapper = jQuery(this);
        var $parentRow = $wrapper.closest('tr');
      ...
      if ($heading.hasClass('pe-main-section-header')) {
        var _$nextMainHeader = $heading.nextAll('.pe-main-section-header').first();
        if (_$nextMainHeader.length > 0) {
          // Include everything between this main header and the next, excluding the next main header itself
          var _$contentBetween = $heading.nextUntil(_$nextMainHeader);
          $allFields = $allFields.add(_$contentBetween);
        } else {
          // If no next main header, take everything following except other main headers
          var $allFollowing = $heading.nextAll();
          var $otherMainHeaders = $allFollowing.filter('.pe-main-section-header');
      ...
      if ($allFields.length === 0) {
        var $nextHeading = $heading.nextAll('.pe-label-subcategory-heading, .pe-main-section-header').first();
      ...
      logger.log("Found ".concat($allFields.length, " fields for section ").concat(path, " using combined strategies"));
      logger.log("Heading classes: ".concat($heading.attr('class')));
      logger.log("Indicator found: ".concat($indicator.length > 0));
      if (visible) {
        $heading.addClass('expanded');
        $indicator.text('▼ ');
        $allFields.show().css('display', '');
        logger.log("Expanded section: ".concat(path));
        ...
        $indicator.text('▶ ');
        $allFields.hide();
        logger.log("Collapsed section: ".concat(path));
    ...
    value: function goToLabelField(path) {
      ...
      var pathParts = path.split('.');
      var verticalTabId = pathParts[0]; // This should be the vertical tab ID (e.g., 'estimate_management')
      var currentTabId = this.getCurrentSubTabId();
      logger.log("DEBUG: Path parts: ".concat(JSON.stringify(pathParts)));
      logger.log("DEBUG: Vertical tab ID: ".concat(verticalTabId));
      logger.log("DEBUG: Current tab ID: ".concat(currentTabId));

      // Always try to switch to the correct tab (force it to ensure proper state)
      logger.log("DEBUG: Switching from tab '".concat(currentTabId, "' to tab '").concat(verticalTabId, "'"));
    ...
    value: function expandSectionsAndNavigateToField(path, pathParts) {
      ...
      logger.log("DEBUG: Looking for field with path: ".concat(path, ", found: ").concat($field.length));
      if ($field.length) {
        ...
        var $fieldWrapper = $field.closest('.pe-label-field-wrapper');
        if ($fieldWrapper.length) {
          var $parentSection = $fieldWrapper.prevAll('.pe-label-subcategory-heading, .pe-main-section-header').first();
          if ($parentSection.length) {
            var sectionPath = $parentSection.next('.pe-label-subcategory-data').data('path') || $parentSection.data('path');
            if (sectionPath && !this.expandedSections.has(sectionPath)) {
              logger.log("DEBUG: Expanding immediate parent section for field: ".concat(sectionPath));
        ...
        logger.warn("DEBUG: Field with path \"".concat(path, "\" not found"));

        // Debug: let's see what fields actually exist
        var allFields = jQuery('input[data-path]');
        logger.log("DEBUG: All available fields:");
        allFields.each(function (i, field) {
          logger.log("  - ".concat(jQuery(field).data('path')));
        });
      }
    }
    ...
    value: function handleClearCaches() {
      var _this16 = this;
      var button = this.$('#clear-label-caches');
      ...
      _utils__WEBPACK_IMPORTED_MODULE_7__.ajax.ajaxRequest({
        ...
        _this16.showNotice(data.message || 'Label caches cleared successfully', 'success');

        // Show user-friendly summary
        var clearedCount = data.cleared_caches ? data.cleared_caches.filter(function (cache) {
          return cache.cleared;
        }).length : 0;
        var hadFlat = data.had_flat_structure;
        var detailMessage = "Cleared ".concat(clearedCount, " cache entries.");
        if (hadFlat) {
          detailMessage += ' Removed legacy flat structure.';
        }
        if (data.new_version) {
          detailMessage += " Version updated to ".concat(data.new_version, ".");
        }
        _this16.showNotice(detailMessage, 'info');
        ...
        _this16.showNotice('Error clearing caches: ' + (error.message || 'Unknown error'), 'error');
      })["finally"](function () {
...
}(_common_VerticalTabbedModule__WEBPACK_IMPORTED_MODULE_8__["default"]); // Initialize the module
jQuery(document).ready(function () {
