...
var RoomManager = /*#__PURE__*/function () {
  ...
  return (0,_babel_runtime_helpers_createClass__WEBPACK_IMPORTED_MODULE_4__["default"])(RoomManager, [{
      ...
      if (room.products) {
        Object.values(room.products).forEach(function (product) {
          if (product.additional_products && Array.isArray(product.additional_products)) {
            product.additional_products.forEach(function (include) {
              var productId = include.id || include.product_id;
    ...
    value: function renderRoomIncludes(room, roomElement) {
      ...
      var products = Array.isArray(room.products) ? room.products : Object.values(room.products);

      // Render each product as an include item
      products.forEach(function (product, index) {
        var price = product.price || product.regular_price || 0;
        var productData = {
      ...
      logger.log("Rendered ".concat(products.length, " products in room includes"));
    }
    ...
    value: function bindIncludeItemRemoveButtons(includesContainer, estimateId, roomId) {
      ...
      removeButtons.forEach(function (button) {
        ...
        button._clickHandler = function (e) {
          ...
          if (_this4.modalManager && _this4.modalManager.confirmationDialog) {
            _this4.modalManager.confirmationDialog.show({
              title: 'Remove Product',
              ...
              onConfirm: function onConfirm() {
                ...
                if (_this4.modalManager.productManager) {
                  ...
                  _this4.modalManager.productManager.performProductRemoval(estimateId, roomId, null, productId);
                }
              },
              onCancel: function onCancel() {
                logger.log('Product removal cancelled by user');
      ...
      upgradeButtons.forEach(function (button) {
        ...
        button._clickHandler = function (e) {
          ...
          if (_this5.modalManager.productManager) {
            ...
            _this5.modalManager.productManager.replaceProductInRoom(estimateId, roomId, replaceProductId,
      ...
      if (toggleButton && similarProductsContainer) {
        ...
        toggleButton._clickHandler = function (e) {
          ...
          if (isExpanded) {
            ...
            if (carouselContainer && _this6.modalManager.uiManager) {
              _this6.modalManager.uiManager.initializeCarouselInContainer(carouselContainer);
            }
          }
        };
        toggleButton.addEventListener('click', toggleButton._clickHandler);
    ...
    value: function bindRoomEvents(roomElement, estimateId, roomId) {
      ...
      if (accordionHeader) {
        ...
        accordionHeader._clickHandler = function (e) {
          ...
          if (isExpanded) {
            ...
            // Initialize similar products for the room
            _this7.initializeSimilarProductsForRoom(estimateId, roomId);
          }
        };
      ...
      if (removeButton) {
        ...
        removeButton._clickHandler = function (e) {
          e.preventDefault();
          e.stopPropagation();
          _this7.handleRoomRemoval(estimateId, roomId);
        };
      ...
      if (addProductButton) {
        ...
        addProductButton._clickHandler = function (e) {
          ...
          if (_this7.modalManager.productManager) {
            _this7.modalManager.productManager.showProductSelection(estimateId, roomId);
          } else {
    ...
    value: function bindNewRoomFormEvents(formElement, estimateId) {
      ...
      formElement._submitHandler = function (e) {
        ...
        var roomLength = formData.get('room_length');
        if (!roomName) {
          _this8.modalManager.showError('Please enter a room name.');
          _this8.modalManager.hideLoading();
          return;
        }
        if (!roomWidth || !roomLength) {
          _this8.modalManager.showError('Please enter room dimensions.');
          _this8.modalManager.hideLoading();
          return;
        }

        // Create the room
        _this8.dataService.addNewRoom({
          room_name: roomName,
          room_width: roomWidth,
          room_length: roomLength
          ...
          if (productId) {
            ...
            if (_this8.modalManager.productManager) {
              ...
              return _this8.modalManager.productManager.addProductToRoom(estimateId, newRoom.room_id, productId).then(function () {
                // Hide loading
                _this8.modalManager.hideLoading();

                // First show the estimates list with the newly created room expanded
                if (_this8.modalManager.estimateManager) {
                  _this8.modalManager.estimateManager.showEstimatesList(estimateId, newRoom.room_id);
                }

                // Then show a confirmation dialog using ConfirmationDialog (with slight delay to allow UI to render)
                if (_this8.modalManager && _this8.modalManager.confirmationDialog) {
                  setTimeout(function () {
                    _this8.modalManager.confirmationDialog.show({
                      title: 'Room Created',
              ...
              // First show the estimates list with the newly created room expanded
              if (_this8.modalManager.estimateManager) {
                _this8.modalManager.estimateManager.showEstimatesList(estimateId, newRoom.id);
              }
            ...
            if (_this8.modalManager.estimateManager) {
              ...
              if (_this8.modalManager && _this8.modalManager.confirmationDialog) {
                _this8.modalManager.confirmationDialog.show({
                  ...
                  confirmText: 'OK',
                  onConfirm: function onConfirm() {
                    _this8.modalManager.closeModal();
                  }
                });
              } else {
                logger.log('Room created successfully.');
          ...
          logger.error('Error creating room:', error);
          _this8.modalManager.showError('Error creating room. Please try again.');
          _this8.modalManager.hideLoading();
        });
      };
      ...
      if (cancelButton) {
        ...
        cancelButton._clickHandler = function (e) {
          ...
          if (productId) {
            // If we're adding a product, go back to estimate selection
            if (_this8.modalManager.estimateManager) {
              _this8.modalManager.estimateManager.showEstimateSelection(productId);
            } else {
              _this8.modalManager.closeModal();
            }
          } else {
            // If we're just creating a room, go back to the estimates list
            if (_this8.modalManager.estimateManager) {
              _this8.modalManager.estimateManager.showEstimatesList();
            } else {
              _this8.modalManager.closeModal();
            }
          }
    ...
    value: function handleRoomRemoval(estimateId, roomId) {
      ...
      this.modalManager.confirmationDialog.show({
        ...
        onConfirm: function onConfirm() {
          ...
          _this9.performRoomRemoval(estimateId, roomId);
        },
        onCancel: function onCancel() {
          // User cancelled, do nothing
      ...
      this.dataService.removeRoom(estimateId, roomId).then(function () {
        ...
        if (roomElement) {
          ...
          if (_this0.modalManager.estimateManager) {
            _this0.modalManager.estimateManager.showEstimatesList(null, estimateId);
          }
        }

        // Update estimate totals and check for empty state
        _this0.dataService.getEstimate(estimateId).then(function (estimate) {
        ...
        if (_this0.modalManager && _this0.modalManager.confirmationDialog) {
          ...
          // Fallback to modalManager.showError
          _this0.modalManager.showError('Error removing room. Please try again.');
        }
        _this0.modalManager.hideLoading();
      });
    }
    ...
    value: function initializeSimilarProductsForRoom(estimateId, roomId) {
      ...
      if (!roomElement) {
        ...
        setTimeout(function () {
          _this1.initializeSimilarProductsForRoom(estimateId, roomId, forceRefresh);
        }, 100);
        return;
      }
    ...
    value: function loadSimilarProductsForRoom(estimateId, roomId, productIds, roomArea) {
      ...
      productsMissingSimilar.forEach(function (productId) {
        var promise = _this10.dataService.getSimilarProducts(productId, roomArea).then(function (similarProducts) {
      ...
      Promise.all(similarProductPromises).then(function () {
        ...
        _this10.renderSimilarProductsList(similarProductsContainer, toggleButton, similarProductsList, estimateId, roomId, productIds, roomElement);
      })["catch"](function (error) {
        ...
        if (lastSimilarItem) {
          ...
          if (replaceButton) {
            replaceButton.addEventListener('click', function (e) {
              ...
              if (_this11.modalManager && _this11.modalManager.productManager) {
                // Check if product has variations before replacing
                _this11.modalManager.productManager.handleProductVariationSelection(productId, {
                  action: 'replace',
                  estimateId: estimateId,
                  roomId: roomId,
                  replaceProductId: replaceProductId,
...
/* harmony import */ var _TemplateEngine__WEBPACK_IMPORTED_MODULE_34__ = __webpack_require__(/*! ./TemplateEngine */ "./src/js/frontend/TemplateEngine.js");
...
function initializeTemplates() {
  ...
  Object.entries(templates).forEach(function (_ref) {
    ...
    if (!html || html.trim() === '') {
      ...
      _TemplateEngine__WEBPACK_IMPORTED_MODULE_34__["default"].registerTemplate(id, html);
    }
  });
  logger.log("Initialized template engine with ".concat(Object.keys(templates).length, " templates"));
  logger.groupEnd();
  return _TemplateEngine__WEBPACK_IMPORTED_MODULE_34__["default"];
}
