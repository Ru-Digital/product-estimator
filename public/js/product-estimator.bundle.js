  ...
  return (0,_babel_runtime_helpers_createClass__WEBPACK_IMPORTED_MODULE_1__["default"])(SuggestionsCarousel, [{
    ...
    value: function getContainerWidth() {
      if (this.isSimilarProducts) {
        ...
        var parentItem = this.container.closest('.room-item');
        if (parentItem) {
    ...
    value: function setContainerSize() {
      ...
      if (this.isSimilarProducts) {
        ...
        var parentEl = this.container.closest('.room-item');
        if (parentEl) {
    ...
    value: function getAvailableWidth() {
      ...
      if (this.isSimilarProducts) {
        ...
        var parentRoomItem = this.container.closest('.room-item');
        if (parentRoomItem) {
          // Get room-item width and consider its padding
          var roomItemStyles = window.getComputedStyle(parentRoomItem);
          var roomItemWidth = parentRoomItem.clientWidth - (parseFloat(roomItemStyles.paddingLeft) + parseFloat(roomItemStyles.paddingRight));
      ...
      suggestionsToggleButtons.forEach(function (button) {
        if (!button._toggleBound) {
          button._toggleHandler = function (e) {
            e.preventDefault();
            e.stopPropagation();
            _this.toggleSuggestions(button);
          };
          button.addEventListener('click', button._toggleHandler);
          button._toggleBound = true;
        }
