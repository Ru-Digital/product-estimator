 ...
 * - Negated conditionals: {{#if !variable}}...{{/if}}
 * - Equality checks: {{#if variable == "value"}}...{{/if}}
 * - Loops: {{#each items}}...{{/each}}
 * - Special loop variables: {{@index}}, {{@first}}, {{@last}}, {{this}}
 * - Object property access: {{user.name}}, {{address.city}}
 */
...
class TemplateEngine {
  ...
  /**
   * Check if a string contains handlebars blocks (conditionals or loops)
   * @param {string} str - The string to check
   * @returns {boolean} - True if the string contains handlebars blocks
   * @private
   */
  hasHandlebarsBlocks(str) {
    if (!str || typeof str !== 'string') {
      return false;
    }
    
    // Check for if blocks or each blocks
    return /\{\{#(if|each)\s+([^}]+)\}\}/i.test(str);
  }
   ...
   * @param {string} str - The string containing {{#if}} blocks
   * @param {object} data - Data object with values to check
   * @returns {string} - The processed string with conditionals evaluated
   * @private
   */
  _processConditionals(str, data) {
    if (!str || typeof str !== 'string') {
      return str;
    }
    
    // Process simple if statements: {{#if variable}}content{{/if}}
    // And negated if statements: {{#if !variable}}content{{/if}}
    // And equality checks: {{#if variable == "value"}}content{{/if}}
    ...
    while ((ifMatch = ifRegex.exec(str)) !== null) {
      ...
      // Check if it's a negation condition
      if (condition.trim().startsWith('!')) {
        const variable = condition.trim().substring(1).trim();
        const value = this._getNestedValue(data, variable);
        const shouldRender = !value;
        processedStr = processedStr.replace(fullMatch, shouldRender ? content : '');
      }
      // Check if it's an equality check
      else if (condition.includes('==')) {
        const [variable, expectedValue] = condition.split('==').map(part => part.trim());
        const actualValue = this._getNestedValue(data, variable);
        
        // Remove quotes from expected value if it's a string literal
        let cleanExpectedValue = expectedValue;
        if ((expectedValue.startsWith('"') && expectedValue.endsWith('"')) || 
            (expectedValue.startsWith("'") && expectedValue.endsWith("'"))) {
          cleanExpectedValue = expectedValue.substring(1, expectedValue.length - 1);
        }
        
        const shouldRender = actualValue == cleanExpectedValue; // Intentional loose comparison
        processedStr = processedStr.replace(fullMatch, shouldRender ? content : '');
      }
      // Simple existence check
      else {
        const value = this._getNestedValue(data, condition.trim());
        const shouldRender = !!value;
        processedStr = processedStr.replace(fullMatch, shouldRender ? content : '');
      }
    }
  ...
  /**
   * Process loops in a template string
   * @param {string} str - The string containing {{#each}} blocks
   * @param {object} data - Data object containing arrays to iterate
   * @returns {string} - The processed string with loops expanded
   * @private
   */
  _processLoops(str, data) {
    if (!str || typeof str !== 'string') {
      return str;
    }
    ...
    while ((eachMatch = eachRegex.exec(str)) !== null) {
      const [fullMatch, arrayPath, itemTemplate] = eachMatch;
      
      // Get the array to iterate from the data object
      const arrayToIterate = this._getNestedValue(data, arrayPath.trim());
      
      if (Array.isArray(arrayToIterate) && arrayToIterate.length > 0) {
        // Process each item in the array
        const processedItems = arrayToIterate.map((item, index) => {
          // Create an item context with special variables
          const itemContext = {
            ...item,
            '@index': index,
            '@first': index === 0,
            '@last': index === arrayToIterate.length - 1,
            'this': item
          };
          
          // Process the item template with this context
          let result = this.processHandlebars(itemTemplate, itemContext);
          
          // Process any conditionals within this item
          result = this._processConditionals(result, itemContext);
          
          return result;
        }).join('');
        
        processedStr = processedStr.replace(fullMatch, processedItems);
      } else {
        // If array is empty or not found, replace with empty string
        processedStr = processedStr.replace(fullMatch, '');
      }
    }
  ...
  processHandlebars(str, data) {
    ...
    let processedStr = this._processConditionals(str, data);
    
    // Then process loops
    processedStr = this._processLoops(processedStr, data);
    
    // Finally process simple variable replacements
    // Enhanced to support nested property access with dot notation
    return processedStr.replace(/\{\{([^#\/][^}]*)\}\}/g, (match, key) => {
      const trimmedKey = key.trim();
      
      // Handle special variables in loops
      if (trimmedKey === '@index' || trimmedKey === '@first' || trimmedKey === '@last') {
        return data[trimmedKey] !== undefined ? data[trimmedKey] : '';
      }
      
      // Handle "this" reference in loops
      if (trimmedKey === 'this') {
        return typeof data === 'object' ? JSON.stringify(data) : String(data);
      }
      
      // Handle nested properties with dot notation
      return this._getNestedValue(data, trimmedKey) !== undefined ? 
        this._getNestedValue(data, trimmedKey) : '';
    });
  }
  ...
  _processElementHandlebars(element, data) {
    ...
    const processNode = (node) => {
      ...
      if (node.nodeType === Node.TEXT_NODE) {
        if (node.textContent && node.textContent.includes('{{')) {
          node.textContent = this.processHandlebars(node.textContent, data);
        }
        return;
      }
      ...
      if (node.hasAttributes()) {
        Array.from(node.attributes).forEach(attr => {
          // Process attributes that contain handlebars
          if (attr.value.includes('{{')) {
            attr.value = this.processHandlebars(attr.value, data);
          }
          
          // Handle special data-if attribute
          if (attr.name === 'data-if') {
            ...
            if (condition.startsWith('!')) {
              ...
              shouldRender = actualValue == cleanExpectedValue; // Intentional loose comparison
            } else {
              // Simple existence check
              const value = this._getNestedValue(data, condition.trim());
              shouldRender = !!value;
            ...
            // Remove the data-if attribute
            node.removeAttribute('data-if');
          }
          
          // Handle special data-each attribute
          if (attr.name === 'data-each') {
            const arrayPath = attr.value;
            const arrayToIterate = this._getNestedValue(data, arrayPath.trim());
            
            if (Array.isArray(arrayToIterate) && arrayToIterate.length > 0) {
              // Clone the node for each item (except the first one)
              const template = node.cloneNode(true);
              const parent = node.parentNode;
              
              // Process first item in the original node
              const firstItemData = {
                ...data,
                ...arrayToIterate[0],
                '@index': 0,
                '@first': true,
                '@last': arrayToIterate.length === 1,
                'this': arrayToIterate[0]
              };
              
              // Process the first node with the first item's data
              node.removeAttribute('data-each');
              processNodeWithData(node, firstItemData);
              
              // Process the rest of the items
              for (let i = 1; i < arrayToIterate.length; i++) {
                const itemData = {
                  ...data,
                  ...arrayToIterate[i],
                  '@index': i,
                  '@first': i === 0,
                  '@last': i === arrayToIterate.length - 1,
                  'this': arrayToIterate[i]
                };
                ...
                // Process the clone with the item's data
                processNodeWithData(clone, itemData);
              ...
              // If array is empty or not found, hide the element
              node.style.display = 'none';
            }
            
            // Remove the data-each attribute
            node.removeAttribute('data-each');
          }
        });
      }
      
      // Process node's inner HTML if it contains handlebars templates
      // This only processes direct text content, not complex children
    ...
    // Helper function to process a node with specific data
    const processNodeWithData = (node, nodeData) => {
      // Process attributes
      if (node.hasAttributes()) {
        Array.from(node.attributes).forEach(attr => {
          if (attr.value.includes('{{')) {
            attr.value = this.processHandlebars(attr.value, nodeData);
          }
        });
      }
      
      // Process text content if a leaf node
      if (node.childNodes.length === 1 && node.firstChild.nodeType === Node.TEXT_NODE) {
        const text = node.textContent;
        if (text.includes('{{')) {
          node.textContent = this.processHandlebars(text, nodeData);
        }
      } else {
        ...
        Array.from(node.childNodes).forEach(child => {
          if (child.nodeType === Node.TEXT_NODE) {
            if (child.textContent && child.textContent.includes('{{')) {
              child.textContent = this.processHandlebars(child.textContent, nodeData);
            }
          } else if (child.nodeType === Node.ELEMENT_NODE) {
            processNodeWithData(child, nodeData);
          }
        });
      }
    };
